...
def optimize_one(spec: RunSpec, cfg: RunConfig, arcface, editor, device, output_dir: Path) -> dict[str, Any]:
    ...
    if done_path.exists() and not cfg.force:
        ...
        if summary_path.exists():
            print(f"[face3] skip completed run: {output_dir}")
    ...
    print(f"[face3] running {spec.slug} image={image_path}")
    ...
    print(f"[face3] generating fixed clean edit reference: prompt={spec.case.prompt!r} steps={cfg.edit_steps}")
    with torch.no_grad():
    ...
    if not cfg.skip_deepface:
        from face3.evaluation.deepface_panel import run_deepface_panel, write_identity_panel
