...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize FACE3 result folders.")
    parser.add_argument("--results-root", default="outputs/edited_output_identity_dct_image")
    parser.add_argument("--output-root", default="outputs/reports/edited_output_identity_dct_image")
...
def build_html(data: dict[str, Any]) -> str:
    ...
    """
    parts = [
        "<!doctype html><html><head><meta charset='utf-8'><title>FACE3 report</title><style>",
        css,
...
def make_pdf(data: dict[str, Any], output_root: Path, pdf_path: Path, compress_images: bool) -> None:
    ...
    p(f"Author: {AUTHOR}")
    p(
        "FACE3 optimizes <b>Z = cosine_similarity</b> between ArcFace embeddings of "
