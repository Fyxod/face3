"""Repair FACE3 public edited-output images with stock InstructPix2Pix replay.

FACE3 optimizes through a differentiable reconstruction of InstructPix2Pix so
...
from face3.core.identity import identity_objective, prepare_identity_reference
from face3.core.image_metrics import image_metrics, pil_to_tensor, save_sheet
from face3.models.arcface import ArcFaceIResNet100


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fix FACE3 saved public edit images using stock InstructPix2Pix replay.")
...
def _latest_run_root(results_root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No FACE3 run roots found under {results_root}")
...
def _case_run_dirs(run_root: Path) -> list[Path]:
    ...
    if not root.exists():
        raise FileNotFoundError(f"Missing FACE3 case root: {root}")
    ...
    if checkpoint is None:
        for run_dir in case_dirs:
            ...
            if checkpoint:
                break
    if not checkpoint:
        print("[face3-fix] ArcFace checkpoint unavailable; stock Z fields will be skipped.")
        return None
    path = Path(checkpoint)
    if not path.exists():
        print(f"[face3-fix] ArcFace checkpoint missing: {path}; stock Z fields will be skipped.")
        return None
        ...
        summary.update(
            {
                ...
                "public_edit_images_fixed_by": "face3.scripts.fix_public_stock_edits",
...
def main() -> None:
    ...
    if not case_dirs:
        raise RuntimeError(f"No FACE3 case directories found under {run_root}")
    ...
    print(f"[face3-fix] run root: {run_root}")
    print(f"[face3-fix] repaired: {sum(row.get('status') == 'repaired' for row in rows)}")
    print(f"[face3-fix] skipped: {sum(row.get('status') == 'skipped' for row in rows)}")
    if args.dry_run:
        print("[face3-fix] dry run only; no files were overwritten.")
    else:
        print(f"[face3-fix] wrote: {out_dir}")
