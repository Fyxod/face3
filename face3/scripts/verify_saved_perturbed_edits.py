"""Replay saved FACE3 perturbed inputs through stock InstructPix2Pix.
...
stock outputs against the differentiable-wrapper outputs saved by FACE3.
...
from face3.core.image_metrics import image_metrics, save_sheet


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay saved FACE3 perturbed images through stock InstructPix2Pix.")
    parser.add_argument(
        "--results-root",
    ...
    parser.add_argument(
        "--run-root",
        default=None,
        help="Specific FACE3 top-level run folder, e.g. outputs/smoke_timing/20260706_181028_edited_output_identity_all_sequential.",
    )
...
def _case_run_dirs(run_root: Path) -> list[Path]:
    ...
    if not root.exists():
        raise FileNotFoundError(f"Missing FACE3 run case root: {root}")
...
def main() -> None:
    ...
    print(f"[face3-verify] wrote {verify_root}")
