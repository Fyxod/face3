...
from face3.core.geometry.combined_face import CombinedFacePerturbation, FaceGeometryConfig, load_face_geometry_config
from face3.core.image_metrics import pil_to_tensor, tensor_to_pil
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Input image path. If not provided, uses face_002 from perturbation_samples.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    if args.image:
        ...
        image_path = root / "perturbation_samples" / "face_005" / "original.png"
        if not image_path.exists():
            # Try to find any sample image
            sample_dir = root / "perturbation_samples" / "face_002"
            if sample_dir.exists():
                image_path = sorted(sample_dir.glob("*.png"))[0]
            else:
                raise FileNotFoundError(
                    "No input image found. Provide --image or ensure perturbation_samples/face_002/original.png exists."
                )
